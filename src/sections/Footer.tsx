    ...
    <footer className="bg-arkos-abyss text-white/70">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {['All Products', 'NanoCBD Tinctures', 'Pain Relief', 'Sleep Support', 'Pet Wellness'].map((item) => (
                <li key={item}>
                  <button onClick={() => scrollTo('products')} className="font-body text-sm text-white/50 hover:text-arkos-light transition-colors">{item}</button>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {['About Us', 'The Science', 'Journal', 'Contact'].map((item) => (
                <li key={item}>
                  <button onClick={() => scrollTo(item === 'The Science' ? 'science' : item === 'About Us' ? 'trust' : 'products')} className="font-body text-sm text-white/50 hover:text-arkos-light transition-colors">{item}</button>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {['Privacy Policy', 'Refund Policy', 'Shipping Policy', 'Terms of Service'].map((item) => (
                <li key={item}><span className="font-body text-sm text-white/50 hover:text-arkos-light transition-colors cursor-pointer">{item}</span></li>
              ))}
            </ul>
          </div>
        </div>
        ...
        <div className="mt-12 pt-6 border-t border-white/10 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <p className="font-body text-[10px] text-white/30 max-w-md text-center sm:text-right">
